...
const charCode = (arr) => {
  let array=arr.map(value => value.charCodeAt());
  return array;
};
...
const extractAbilities = (arr) => {
  let array=arr.map(value => value.ability.name );
  return array;
};
...
const extractStats = (arr) => {
  let array=arr.map(value => ({name:value.stat.name,total:value.effort+value.baseStat}));
  return array;
};
